...
app.factory('userManagementService',function($http){
	...
	return {
		getAllUser: function(){
			return $http.get('http://localhost:8080/getAllUser');
		},
...
app.factory('droneManagementService',function($http){
	return {
		getAllDrone : function(){
			return $http.get("http://localhost:8080/getAllDrone");
		},
		deleteDrone: function(code){
            return $http.get(
                            "http://localhost:8080/deleteDrone",{params:{
                            	code:code
                            }});
        },
        insertDrone: function(drone){
            return $http.get("http://localhost:8080/insertDrone",
                                {
                                    params:{
                                        code:drone.idcode,
                                        brand:drone.brand,
        ...
        updateDrone: function(drone){
            return $http.get("http://localhost:8080/updateDrone",
                                {
                                    params:{
                                        code:drone.code,
                                        brand:drone.brand,
...
app.controller('userManagement',function($rootScope,$scope, $uibModal, userAccountService,userManagementService){
    ...
    usMng.deleteUser = function(itemindex){
    	if ($rootScope.username != usMng.users[itemindex].username){
        	...
        	usMng.openPopUp('Operation denied the account in use can not be removed.');
        }
    };
    ...
    drMng.insertDrone = function(drone){
        droneManagementService.insertDrone(drone).success(function(data){
            if (data.errorCode == "1") {
                ...
                drMng.openPopUp('Drone crated sussifully');
            }
        });
    };
